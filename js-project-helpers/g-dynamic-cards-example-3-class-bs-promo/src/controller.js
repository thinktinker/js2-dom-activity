...
class Controller{
            ...
            if(data.length > 0){
                for (let index = 0; index < this.currentId; index++) {
                    sampleItems.push({
                        ...
                        scheduled: data[index].scheduled,
                    })
                }
            }
    ...
    displayCart(data){
        ...
        for (let index = 0; index < data.length; index++) {
            const {start, end} = data[index].scheduled;
            const convertedStartDate = new Date(start).getTime();
            const convertedEndDate = new Date(start).getTime();
            // promo
            if(data[index].promo === true && convertedStartDate <= Date.now() && convertedEndDate > Date.now()){ 
